...
var debug = logger( 'to-reference:sync' );
...
function toReference( id, options ) {
	...
	try {
		...
		// If the conversion command (e.g., pandoc) is not available or fails,
		// throw a descriptive error so callers (and doctest checks) can
		// appropriately annotate or handle the failure.
		throw new Error( 'Conversion failed (ensure pandoc and related tools are installed): ' + err.message );
	}
